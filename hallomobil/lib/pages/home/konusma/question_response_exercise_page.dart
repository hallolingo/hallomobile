...
class QuestionResponseExercisePage extends StatefulWidget {
  final String selectedLanguage;
  final String selectedLevel;

  const QuestionResponseExercisePage({
    super.key,
  ...
  @override
  State<QuestionResponseExercisePage> createState() =>
      _QuestionResponseExercisePageState();
}

class _QuestionResponseExercisePageState
    extends State<QuestionResponseExercisePage> with TickerProviderStateMixin {
  ...
  bool _isQuestionCorrect = false; // Tracks if question is correctly pronounced
  bool _isAnswerCorrect = false; // Tracks if answer is correctly pronounced
  ...
  Future<void> _fetchExercises() async {
    try {
      final querySnapshot = await FirebaseFirestore.instance
          .collection('speaking_question_response')
          ...
          setState(() {
            ...
            if (_exercises.isNotEmpty) {
              if (!_isQuestionCorrect) {
                // Question phase: Check if the question is pronounced correctly
                String expectedQuestion = _exercises[_currentExerciseIndex]
                        ['question']
                    .toString()
                    .toLowerCase();
                _isQuestionCorrect = _recognizedText
                    .toLowerCase()
                    .contains(expectedQuestion.split(' ').first.toLowerCase());
                _feedbackMessage = _isQuestionCorrect
                    ? 'Soru doğru! Şimdi yanıtı söyleyin.'
                    : 'Soru yanlış, tekrar deneyin!';
              } else {
                // Answer phase: Check if the answer is pronounced correctly
                String expectedAnswer = _exercises[_currentExerciseIndex]
                        ['sampleAnswer']
                    .toString()
                    .toLowerCase();
                _isAnswerCorrect = _recognizedText
                    .toLowerCase()
                    .contains(expectedAnswer.split(' ').first.toLowerCase());
                _feedbackMessage = _isAnswerCorrect
                    ? 'Doğru!'
                    : 'Yanıt yanlış, tekrar deneyin!';
              }
            } else {
              _feedbackMessage = 'Soru bulunamadı!';
            }
            debugPrint(
                'Feedback: $_feedbackMessage, isQuestionCorrect: $_isQuestionCorrect, isAnswerCorrect: $_isAnswerCorrect');
          });
        },
  ...
  void _markAsCorrect() {
    setState(() {
      if (!_isQuestionCorrect) {
        _isQuestionCorrect = true;
        _feedbackMessage = 'Soru doğru (Manuel onay)! Şimdi yanıtı söyleyin.';
      } else {
        _isAnswerCorrect = true;
        _feedbackMessage = 'Doğru (Manuel onay)!';
      }
      _recognizedText = _isQuestionCorrect && !_isAnswerCorrect
          ? _exercises[_currentExerciseIndex]['question'].toString()
          : _exercises[_currentExerciseIndex]['sampleAnswer'].toString();
    });
                                ...
                                children: [
                                  ...
                                  Row(
                                    children: [
                                      Expanded(
                                        child: Text(
                                          'Soru: ${_exercises[_currentExerciseIndex]['question']}',
                                  ...
                                  Row(
                                    children: [
                                      Expanded(
                                        child: Text(
                                          'Türkçe: ${_exercises[_currentExerciseIndex]['questionTurkish']}',
                                          style: TextStyle(
                                            fontSize: 16,
                                            color: ColorConstants.TEXT_COLOR
                                                .withOpacity(0.7),
                                  ...
                                  if (_isQuestionCorrect) ...[
                                    Row(
                                      children: [
                                        Expanded(
                                          child: Text(
                                            'Örnek Yanıt: ${_exercises[_currentExerciseIndex]['sampleAnswer']}',
                                            style: TextStyle(
                                              fontSize: 16,
                                              color: ColorConstants.TEXT_COLOR
                                                  .withOpacity(0.7),
                                    ...
                                    Row(
                                      children: [
                                        Expanded(
                                          child: Text(
                                            'Türkçe Yanıt: ${_exercises[_currentExerciseIndex]['sampleAnswerTurkish']}',
                                            style: TextStyle(
                                              fontSize: 16,
                                              color: ColorConstants.TEXT_COLOR
                                                  .withOpacity(0.7),
